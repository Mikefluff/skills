"""Thumbnail batch execution CLI — same shape as cover/avatar/flyer.
...
def main() -> int:
    parser = argparse.ArgumentParser(prog="common.runners.cli.thumbnail")
    ...
    if plan.get("schema") != "skills.thumbnail.plan.v1":
    ...
    output_dir = Path(plan.get("output_dir") or f"./generated/thumbnail/{plan.get('slug') or 'batch'}")
    ...
    print(
        f"Thumbnail: {len(items)} variant(s) via {model} "
    ...
    print(f"\nThumbnail: {output_dir}  ({succeeded}/{len(items)} variant(s) succeeded)", file=sys.stderr)
