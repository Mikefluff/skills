...
def main() -> int:
    ...
    offer_text = _read_offer(args)
    plan = parse_mod.parse(offer_text)
    if args.currency:
        plan["currency"] = args.currency.upper()
        for it in plan["items"]:
            it["currency"] = it["currency"] or plan["currency"]
    ...
    # data-quality warnings (surfaced to the user in both modes)
    ...
    for o in plan.get("price_outliers", []):
        print(f"⚠ '{o['name']}' is {o['share']*100:.0f}% of the total "
              f"({o['price']}) — possible typo; confirm with the user.", file=sys.stderr)
    ...
    # Pick a photo for any service that has none — generate an on-brand image.
    missing = [it["name"] for it in plan["items"] if not it.get("thumb")]
    if missing and not args.no_gen_photos:
        if kit_mod._pick_image_provider() is not None:
            print(f"… {len(missing)} item(s) without a photo — generating on-brand "
                  f"images: {', '.join(missing)}", file=sys.stderr)
            g = kit_mod.fill_missing_photos(
            ...
            print(f"… generated {g} photo(s) (AI, on-brand — swap for a real one if you "
    ...
    # -------------------------------------------------- default: brand-kit for LLM authoring
    if not args.quick:
        ...
        manifest = {
            "skill": "proposal-maker", "mode": "kit", "slug": slug, "lang": lang,
            "brand_url": brand.get("url"), "brand_ok": brand.get("ok"),
            "accent": brand.get("accent"), "font": brand.get("font_heading"),
            "screenshot": shot_ok, "logo_local": logo_local.name if logo_local else None,
            ...
            "thumbnails": not args.no_thumbnails, "files": files,
        }
