"""Quote-card batch execution CLI — same shape as cover.py.
...
def main() -> int:
    parser = argparse.ArgumentParser(prog="common.runners.cli.quote")
    ...
    if plan.get("schema") != "skills.quote.plan.v1":
    ...
    output_dir = Path(plan.get("output_dir") or f"./generated/quote/{plan.get('slug') or 'batch'}")
    ...
    print(
        f"Quote: {len(items)} card(s) via {model} "
    ...
    print(f"\nQuote: {output_dir}  ({succeeded}/{len(items)} card(s) succeeded)", file=sys.stderr)
    print(str(output_dir))
    if failed:
        print(f"  {failed} card(s) failed. Re-run with --resume to retry.", file=sys.stderr)
        return 1
    return 0
