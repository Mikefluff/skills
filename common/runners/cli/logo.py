"""Logo batch execution CLI — same shape as cover.py.
...
def main() -> int:
    parser = argparse.ArgumentParser(prog="common.runners.cli.logo")
    ...
    if plan.get("schema") != "skills.logo.plan.v1":
    ...
    output_dir = Path(plan.get("output_dir") or f"./generated/logo/{plan.get('slug') or 'batch'}")
    ...
    extra_meta = {
        ...
        "palette_hint": plan.get("palette_hint"),
        "lang": plan.get("lang"),
        "model": model,
    ...
    print(
        f"Logo: {len(items)} variant(s) via {model} "
    ...
    print(f"\nLogo: {output_dir}  ({succeeded}/{len(items)} variant(s) succeeded)", file=sys.stderr)
