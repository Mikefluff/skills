...
| Scenario | Skill(s) | Walkthrough |
|---|---|---|
| Generate an image prompt for 14+ models (Midjourney V8.1, Flux 2, Nano Banana 2, Nano Banana Pro, gpt-image-2, Ideogram 3, Seedream 5.0, …) | [`image-prompt`](skills/image-prompt/) | [image-prompt-cover](docs/walkthroughs/image-prompt-cover.md) |
...
| [`image-prompt`](skills/image-prompt/) | wrapper | en/ru | Write prompts for 14+ frontier AI image generators (Midjourney V8.1, Flux 2 / Flux Kontext, Nano Banana Pro / 2 / 2 Lite (Gemini image family), gpt-image-2, Ideogram 4 / 3, Recraft V3, Seedream 5.0, Qwen-Image, HiDream-O1, Krea-1, SD 3.5, SDXL). Modes: text-to-image, edit (preserve/change), multi-reference, text-in-image. 6-part formula + per-model deltas, character/identity locks, weighted multi-ref. |
...
| [`schema-maker`](skills/schema-maker/) | wrapper | en | Generate JSON-LD structured data (Article, FAQPage, HowTo, Organization, Person with knowsAbout) and llms.txt from a markdown post. Schema became a citation lever for AI answer engines when Google retired FAQ rich results — engines parse FAQPage first when choosing whose answer to quote. Deterministic and offline. |
...
| [`transcribe-maker`](skills/transcribe-maker/) | wrapper | en/ru | Transcribe audio / video to SRT / WebVTT / JSON / plain text via OpenAI Whisper or the GPT-4o transcribe models. Auto-detects language or accepts ISO-639-1 hint. $0.003-0.006/min. Closes the loop with subtitle-burner — produce captions from video, then burn them in. 25 MB file limit. |
...
| [`voiceover-maker`](skills/voiceover-maker/) | wrapper | en/ru | Text-to-speech skill — script in, MP3 out. Wraps the runner's audio modality (ElevenLabs eleven-tts on Eleven v3 + OpenAI gpt-4o-mini-tts). Supports voice picker, multilingual TTS across 70+ languages, speed control, long-form scripts. Auto-picks provider based on language + script length + brand-voice needs. Saves to ./generated/audio/<timestamp>-<model>.mp3. |
...
Vendors covered: OpenAI (gpt-image-2, Whisper + GPT-4o transcribe, TTS, Sora 2 until its 2026-09-24 removal), Google (Nano Banana Pro / 2 / 2 Lite, Veo 3.1 + Fast + Lite, Lyria 3 Pro), Black Forest Labs (Flux family + Kontext), Runway (Gen-4, Gen-4.5, Aleph), Kuaishou (Kling 3.0), Ideogram (3 Flash/Turbo/Default/Quality), ElevenLabs (Music + TTS on Eleven v3), Suno (v5.5), fal.ai + Replicate routers for everything else (Seedream 5, Hunyuan, LTX-2, Wan, MusicGen, Stable Audio, many open-source).
