"""Neuroslop catalogues — the patterns lint.py matches on.

Split out of lint.py, which had grown past the module-size gate at 988 lines.
This half is data: the probabilistic pattern list, the copy-paste artifacts,
the hard bans, and the small regexes the structural detectors share. lint.py is
the machinery that runs them.

Everything here is re-exported from lint, so `from lint import PATTERNS` and the
31 fixture snapshots that drive it keep working unchanged.
"""
