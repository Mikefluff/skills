...
<objective>
Это БАЗОВЫЙ движок чистой русской/английской прозы. Он не пишет «с нуля» — он задаёт правила, которые соблюдает любой текст, генерируемый под другими скиллами (viral-text, prose-edit, essay-write, главы НК), и проверяет результат до выдачи.
...
Особый кейс: **художественный текст автора** (АБ/ЭА/НК и любые fiction-фрагменты). К нему НЕЛЬЗЯ применять «бизнес-редактирование» — повторы, восклицания, filler-фразы и неполные предложения там часто работают как голос и ритм. Если контекст указывает на художку — действуй консервативно: правь только то, что точно нейрослоп.
