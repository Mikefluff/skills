...
## 5. (RU) ADR — Выбор очереди сообщений для платёжного сервиса

### Контекст

RU-команда платёжного сервиса принимает архитектурное решение: какой брокер использовать для async-обработки webhook'ов от Stripe. Документ ведётся на русском, потому что команда полностью RU-говорящая, peer-review проходит в RU-чате.
...
Используем Kafka. Это лучший выбор, потому что он самый надёжный и мощный.
Также рассмотрели RabbitMQ и SQS, но они нам не подходят. Все согласны.
```

What's wrong:
- Нет статуса, даты, авторов
- «Лучший выбор» / «самый надёжный и мощный» — маркетинговый язык
- «Не подходят» — без объяснения причин
- Нет контекста: какие нагрузки, какие SLA, какие ограничения
- Нет последствий (ни плюсов, ни минусов, ни рисков)
- «Все согласны» — пустая фраза, не заменяет нормальное обоснование

### После (rewrite)

```markdown
# ADR-0012: Использовать AWS SQS (FIFO) для очереди webhook-событий Payments

**Статус**: Принято
**Дата**: 2026-05-20
**Авторы**: А. Петров, Б. Смирнов, В. Иванова

## Контекст

Сервис Payments обрабатывает входящие webhook'и от Stripe. Прогнозируемая нагрузка:
~200 webhook'ов/сек в пиковые часы, ~30/сек в среднем. Требования:

- Гарантированная доставка (at-least-once): потеря webhook'а = ручная сверка с Stripe
- Упорядоченность в пределах одного `payment_intent_id` (повторные события того же
  intent'а должны прийти в порядке отправки)
- Идемпотентность обработки (на стороне consumer'а, через `event_id`)
- SLA по latency обработки: p95 ≤ 5 секунд от получения до записи в БД
- Бюджет на инфраструктуру: ≤ $2 000/месяц

Команда работает в AWS; альтернатива - поднимать managed Kafka (MSK) или self-hosted
RabbitMQ требует операционных расходов, которые мы хотим избежать в первой итерации.

## Решение

Мы будем использовать **AWS SQS FIFO queues** в качестве транспорта для webhook-событий
Payments. Ключевые параметры:

- FIFO-режим с `MessageGroupId = payment_intent_id` для упорядоченности внутри intent'а
- `MessageDeduplicationId = event_id` для дедупликации на стороне SQS
- Visibility timeout - 30 секунд (consumer SHOULD завершать обработку за < 25 с)
- DLQ после 5 неудачных попыток; DLQ-сообщения MUST алертить SRE
- Long polling (20 с) для consumer'ов - снижает стоимость API-вызовов

## Последствия

- **Положительные**:
  - Полностью managed, никакой операционной нагрузки (бэкапы, патчинг, мониторинг)
  - Прайсинг укладывается в бюджет: при 200/сек FIFO стоит ~$300/мес + DLQ ~$50
  - Native интеграция с CloudWatch и Lambda (если позже захотим serverless-consumer)
  - FIFO даёт нужную упорядоченность из коробки

- **Отрицательные**:
  - Vendor lock-in на AWS: миграция в другой облако потребует переписать transport
  - Лимит SQS FIFO: 3 000 сообщений/сек на queue (с batching). При росте до 1 500/сек
    придётся шардировать по queue или мигрировать
  - Меньше observability, чем у Kafka: не хватает offset-замера и re-replay по timestamp

- **Риск**: при превышении 3 000 msg/sec FIFO деградирует. Mitigation: алерт CloudWatch
  при > 2 000 msg/sec sustained 5 минут; план шардирования по `region_code` готов
  заранее.

## Рассмотренные альтернативы

### AWS MSK (managed Kafka)
Лучшая observability, выше throughput-потолок, поддержка replay. Но: стоимость ~$800/мес
для двух брокеров + storage; кривая обучения для команды ~4 недели; для текущих 200/сек
overkill. Возможный возврат к этому варианту при росте > 1 500/сек.

### Self-hosted RabbitMQ
Гибкость в роутинге, привычно команде. Но: операционные расходы на patching,
HA-кластеризацию, бэкапы. Не хочется тащить дополнительный stateful-сервис в Payments
без необходимости.

### Стандартные SQS (не FIFO)
Дешевле, выше throughput. Но не гарантирует упорядоченность внутри intent'а, а это
требование от продукта. Отвергнуто.

### Прямая обработка без очереди (synchronous webhook → DB)
Самое простое. Но: при downtime БД мы теряем webhook'и (Stripe ретраит ~3 раза за 3
дня, но это hard ограничение). Также не позволяет всплески > 200/сек без autoscaling
самого сервиса. Отвергнуто.

## Ссылки

- RFC-0042: Архитектура сервиса Payments
- ADR-0007: Использование PostgreSQL 16+ для Payments
...
- Шапка добавлена полностью: номер ADR, перевод названия в форму «решение + объект», статус, дата (ISO), авторы
- «Лучший выбор» / «самый надёжный и мощный» → удалены, заменены количественными требованиями (200/сек, $2K/мес, p95 ≤ 5 с)
- Контекст добавлен с конкретными цифрами и SLA — необходимо для будущего читателя через 2 года
- Решение конкретное: указан режим (FIFO), параметры (visibility timeout, DLQ retry), наблюдаемые требования
- RFC 2119 keywords (MUST, SHOULD) оставлены КАПСОМ на английском — даже в полностью русском документе (правило из `templates.md` § RU template variants)
- Последствия: и плюсы, и минусы, и риск с mitigation — все три категории
- 4 альтернативы (Kafka, RabbitMQ, обычные SQS, без очереди) с конкретными причинами отказа
- Технические термины (`MessageGroupId`, `event_id`, `visibility timeout`, `DLQ`) — оставлены оригиналом в backticks
- Названия сервисов (AWS SQS, Stripe, CloudWatch, Lambda, MSK, Kafka, RabbitMQ) — оригинал
- Ссылки — смесь внутренних RU-документов и внешних EN-docs
